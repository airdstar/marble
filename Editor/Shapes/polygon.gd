	...
	if has_hole:
		for n : float in sides:
			positions.append_array([
				Vector3(cos(n/sides * PI * 2.0) * (size.x / 2) / hole_size + hole_offset, size.y / 2, sin(n/sides * PI * 2.0) * (size.z / 2) / hole_size + hole_offset) + total_offset,
				Vector3(cos((n + 1)/sides * PI * 2.0) * (size.x / 2) / hole_size + hole_offset, size.y / 2, sin((n + 1)/sides * PI * 2.0) * (size.z / 2) / hole_size + hole_offset) + total_offset,
